...
export function CommandCenterPage() {
  ...
  return <div className="stack">
    <div className="page-head page-head-row"><div><div className="eyebrow">Executive command center</div><h1>A clearer view of what’s next.</h1><Explain label="About this page" variant="note">Demand, model performance and inventory exposure — one connected view of the AIS network.</Explain><div className="page-context"><span>Consumer Glass Solutions</span><span>Branch × SKU × month</span>{forecast.data && <span>Origin {monthLabel(forecast.data.origin_period)}</span>}</div></div><Link to="/forecasts" className="btn btn-primary">Explore forecasts <Icon name="arrow" size={16} /></Link></div>
    ...
    <details className="audit-details"><summary>Service health & technical provenance</summary><div>{health.isPending && <LoadingBlock rows={3} label="Checking service health" />}{health.isError && <ErrorState error={health.error} onRetry={() => health.refetch()} />}{health.data && <div className="table-scroll"><table className="data"><thead><tr><th>Component</th><th>Status</th><th>Detail</th></tr></thead><tbody>{health.data.components.map(component => <tr key={component.name}><td>{component.name}</td><td><HealthPill status={component.status} /></td><td>{component.detail ?? '—'}</td></tr>)}</tbody></table></div>}{registry.isError && <ErrorState error={registry.error} onRetry={() => registry.refetch()} />}<Explain variant="hint">Values are read from the API. Empty or failed stages remain unavailable, never zero-filled. Stock values are snapshot estimates, not real-time balances.</Explain></div></details>
  </div>;
}
