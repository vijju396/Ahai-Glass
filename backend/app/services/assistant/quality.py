...
def check_explanation(text: str, *, must_mention: tuple[str, ...] = ()) -> Verdict:
    ...
    wordlike = sum(1 for t in tokens if _WORD.fullmatch(t.strip(".,;:()[]%—-")))
    if wordlike / len(tokens) < MIN_WORDLIKE_SHARE:
        ...
        if (wordlike + numeric) / len(tokens) < MIN_WORDLIKE_SHARE:
            return Verdict(
                False,
                f"only {wordlike}/{len(tokens)} tokens are words",
            )
